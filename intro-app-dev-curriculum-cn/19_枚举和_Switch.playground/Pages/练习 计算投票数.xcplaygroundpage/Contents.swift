/*:
 ...
 在“数组与循环”playground 中，你有机会创建了一个函数，计算同学的投票数。那时，你只能问可以存放在布尔结果中的是/否问题。
 
 你担心使用字符串可能会导致投票产生拼写错误。但是，现在你已经学习了枚举，可以放心地创建略微复杂的投票系统了：
*/
...
/*:
 ...
 通过非人工计票方式，调查学生更喜欢巧克力工厂还是沙滩。*提示：如需复习如何使用数据集合，可查看“数组与循环”playground。*
 */
...
/*:
 ### 扩展：
 在其他民意调查中（如选择学校吉祥物），你决定添加 `undecided` 选项：
 */
...
/*:
 ...
 - experiment:(体验): 
 在“数组与循环”投票计数练习中，扩展练习要求你编写一个函数来计算任何布尔投票的结果。是什么阻止你编写一个函数来计算 `tripDestinationVotes` 和 `mascotVotes`？
 */

//: [上一页](@previous)  |  第 20 页，共 21 页  |  [下一页：练习：Switch](@next)
